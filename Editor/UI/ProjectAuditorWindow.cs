...
namespace Unity.ProjectAuditor.Editor
{
    ...
    {       
        ...
        private readonly ModeDescriptor[] m_ModeDescriptor = new[]
        {
            new ModeDescriptor
            {
                category = IssueCategory.Allocations,
                name = IssueCategory.Allocations.ToString(),
...
In addition, it is possible to filter issues by area (CPU/Memory/etc...) or assembly name or search for a specific string.";
        ...
        private void OnEnable()
        {
            ...
            m_CategoryNames = m_ModeDescriptor.Select(m => m.name).ToArray();
            ...
            for (int i = 0; i < numColumns; i++)
            {
                ...
                switch ((IssueTable.Column) i)
                {
                    ...
                    case IssueTable.Column.Area :
                        if (modeDescriptor.showAreaColumn)
                        {
                            width = 60;
                            minWidth = 50;
                        }
        ...
        private void RefreshDisplay()
        {
            ...
            if (m_IssueTables.Count == 0)
            {
                ...
                {
                    var issueTable = CreateIssueTable(m_ModeDescriptor[i], new TreeViewState());
